...
Training-free SDE plug-in. Replaces the **white** noise that ``ERSDESampler``
injects each step with **frequency-colored**, RMS-normalized noise that dumps
the fixed stochastic-energy budget into the radial frequency bands the network
has *not yet resolved* at that step (per the precomputed completion matrix
