...
- **`app.py::MainWindow`** — top bar (Guidebook / Models / Update / Queue + TensorBoard overlay toggles / language) + one tab set (ConfigTab, Preprocess, Dataset, Merge, Experimental = MethodsTab picker w/ SPD/Turbo distill, EasyControl) in a `QStackedWidget` with the global TensorBoard and Queue overlay views. Dark `QPalette` via `_dark()`.
